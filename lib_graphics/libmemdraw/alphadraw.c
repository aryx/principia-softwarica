...
writebyte(Param *p, byte *w, Buffer src)
{
    ...
    for(i=0; i<dx; i++){
        ...
        /*
         * claude: the original (still present in 9front and plan9port):
         *
         *      w[0] = u;
         *      w[1] = u>>8;
         *      w[2] = u>>16;
         *      w[3] = u>>24;
         *
         * always writes 4 bytes regardless of nb. For depths < 32 that
         * loop is a read-modify-write: the top bytes were just read
         * above (`u = w[0] | (w[1]<<8) | ...`) and, after masking, get
         * written back unchanged. For interior pixels those bytes are
         * simply the next pixel, so no harm done. But at the LAST pixel
         * of the last row, w[1..3] (for nb=1) or w[2..3] (for nb=2) or
         * w[3] (for nb=3) fall PAST the end of the image allocation
         * and clobber whatever the pool placed next -- typically the
         * boundary tag of the following block. Symptom: an innocent
         * freememimage on an adjacent image later faults deep inside
         * poolfreel / blockcheck / getdsize.
         *
         * Reproducer: resize.c on a 24x25 k8a8 image (nb=2). Image
         * buffer is 25*48 = 1200 bytes; last pixel at offset 1198;
         * w[2..3] at 1200..1201 overflow by 2 bytes and corrupt the
         * adjacent pool block's header. Same bug exists unfixed in
         * 9front and plan9port (no 9legacy patch either).
         *
         * Fix: only write the nb bytes that actually belong to the
         * destination pixel. For nb=4 (RGBA32 etc.) the behavior is
         * identical to before. For nb<4, interior-pixel behavior is
         * still correct because the low nb bytes of u carry the real
         * new pixel value and the high bytes were going to be written
         * back unchanged anyway (we now just skip that self-copy).
         */
        switch(nb){
        case 4: w[3] = u>>24; /* fall through */
        case 3: w[2] = u>>16; /* fall through */
        case 2: w[1] = u>>8;  /* fall through */
        case 1: w[0] = u;     break;
        }
        w += nb;
    }
