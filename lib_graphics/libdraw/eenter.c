...
eenter(char *ask, char *buf, int len, Mouse *m)
{
	...
	bordcol = allocimage(display, Rect(0,0,1,1), view->chan, 1, DPurpleblue);
	...
	while(!done){
		...
		if(r.min.x < view->r.min.x)
			p.x = view->r.min.x - r.min.x;
		if(r.min.y < view->r.min.y)
			p.y = view->r.min.y - r.min.y;
		r = rectaddpt(r, p);
		p = ZP;
		if(r.max.x > view->r.max.x)
			p.x = r.max.x - view->r.max.x;
		if(r.max.y > view->r.max.y)
			p.y = r.max.y - view->r.max.y;
		r = rectsubpt(r, p);
		...
		/* view might have been resized */
		if(b != view || !eqrect(view->clipr, sc)){
			freeimage(save);
			save = nil;
		}
		b = view;
