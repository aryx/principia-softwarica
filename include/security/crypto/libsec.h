...
/*
 * ---------------------------------------------------------------------
 * Principia extensions not present in 9front libsec.h.
 * Preserved because downstream principia code still uses them:
 *   - md5pickle/sha1pickle: state serialization, used by hget(1)
 *     (resumable download feature). Implementations are in
 *     md5pickle.c and sha1pickle.c (principia-local files).
 *   - asn1toDSApriv: DSA private-key parsing, used by
 *     security/auth/asn12dsa.c. Implementation ported into
 *     asn1dsa.c (factored out of the old principia x509.c).
 * ---------------------------------------------------------------------
 */
