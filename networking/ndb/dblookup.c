...
extern ipaddr	ipaddr_;	/* my ip address */
...
lookupinfo(char *attr)
{
    ...
    snprint(buf, sizeof buf, "%I", ipaddr_);
    a[0] = attr;
...
static void
addlocaldnsserver(DN *dp, int class, char *ipaddr_, int i)
{
    int n;
    DN *nsdp;
    RR *rp;
    ...
    if (n < nelem(locdns))
        if (locdns[n] == nil || ++n < nelem(locdns))
            locdns[n] = strdup(ipaddr_); /* remember 1st few local ns */
    qunlock(&locdnslck);
...
//	rp->ttl = 10*Min;		/* seems too short */
    ...
    /* A or AAAA record */
    if (parseip(ip, ipaddr_) >= 0 && isv4(ip))
        rp = rralloc(Ta);
    else
        rp = rralloc(Taaaa);
    rp->ip = dnlookup(ipaddr_, class, 1);
    rp->owner = nsdp;
    rp->local = 1;
    rp->db = 1;
//	rp->ttl = 10*Min;		/* seems too short */
    ...
    dnslog("added local dns server %s at %s", buf, ipaddr_);
}
