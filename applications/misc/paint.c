...
update(Rectangle *rp){
	if(canvas==nil)
		...
		if(rp == nil)
			rp = &canvas->r;
		gendrawdiff(view, view->r, c2sr(canvas->r), back, ZP, nil, ZP, SoverD);
		zoomdraw(view, c2sr(*rp), ZR, back, canvas, rp->min, zoom);
	}
...
expand(Rectangle r)
{
	...
	if(canvas==nil){
		if((canvas = allocimage(display, r, view->chan, 0, DNofill)) == nil)
...
translate(Point d)
{
	...
	r = c2sr(canvas->r);
	nr = rectaddpt(r, d);
	rectclip(&r, view->clipr);
	draw(view, rectaddpt(r, d), view, nil, r.min);
	zoomdraw(view, nr, rectaddpt(r, d), back, canvas, canvas->r.min, zoom);
	gendrawdiff(view, view->r, nr, back, ZP, nil, ZP, SoverD);
	spos = addpt(spos, d);
	flushimage(display, 1);
}
...
center(void)
{
	...
	spos = addpt(view->r.min,
		divpt(subpt(view->r.max, view->r.min), 2));
	update(nil);
}
...
drawpal(void)
{
	...
	for(i=0; i<NBRUSH; i++){
		...
		if(i == NBRUSH-1){
			...
			draw(view, rr, ink, nil, ZP);
		} else {
			rr.min = addpt(rr.min, divpt(subpt(rr.max, rr.min), 2));
			rr.max = rr.min;
			strokedraw(view, rr, ink, i);
		}
		r.min.x = r.max.x;
	}
	...
	for(i=1; i<=nelem(pal); i++){
		...
		if(ink == pal[i-1])
			rr.min.y += Dy(r)/3;
		draw(view, rr, pal[i-1], nil, ZP);
		gendrawdiff(view, r, rr, back, ZP, nil, ZP, SoverD);
		r.min.x = r.max.x;
	}
