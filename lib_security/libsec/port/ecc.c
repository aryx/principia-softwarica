/*
 * Elliptic-curve arithmetic over prime fields, for ECDSA verification
 * and ECDHE with NIST curves.
 *
 * 2026-04-21: principia port. Body copied from 9front-hg-2014
 * sys/src/libsec/port/ecc.c, which does everything in affine
 * coordinates and is self-contained over libmp -- no jacobian helpers,
 * no mpc-generated code. Slower than current 9front's jacobian-based
 * version but avoids the mpc build dependency. Five API shims
 * (ecdominit/ecdomfree/ecencodepub/ecdecodepub/ecpubfree) appended
 * from current 9front's ecc.c so callers in tlshand.c and x509.c get
 * the API they expect. Minus the mpfield(dom->p) Montgomery-field
 * optimization call which principia's libmp does not provide.
 *
 * Caveats:
 *   - ecmul uses double-and-add; not constant-time. Fine for TLS
 *     client verification (verifies public values, no secret inputs).
 *     Do not use for server-side ECDHE without hardening.
 *   - Fixed a typo in the original 2014 ecdsaverify: the fourth guard
 *     compared r twice; corrected to compare s against dom->n.
 */
#include "os.h"
#include <mp.h>
#include <libsec.h>
...
ecdsaverify(ECdomain *dom, ECpub *pub, uchar *dig, int len, mpint *r, mpint *s)
{
	...
	/* claude: fixed 2014 typo -- last guard was mpcmp(r, dom->n) twice,
	 * should be mpcmp(s, dom->n) per SEC1 4.1.4 step 1. */
...
/*
 * API shims merged from current 9front's ecc.c so that tlshand.c and
 * x509.c get the entry points they expect. The mpfield(dom->p) call
 * from 9front's ecdominit is intentionally omitted -- that's a
 * Montgomery-multiplication optimization only present in 9front's
 * libmp, not principia's.
 */
void
