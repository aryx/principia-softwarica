...
(Re)LPization Browser.nw/VCS.nw, 

- TODO LPization abaco/mothra, webfs, webcookies, hget in Browser.nw
  switch from mmm to abaco/mothra (keep mmm in OCaml edition of Browser.nw)
- TODO LPization git9
  switch from ogit to git9 (keep ogit in OCaml edition of VCS.nw)
...
*** try cross-compile git9 and execute git9 from plan9
and then start VCS.nw book! and add on website

*** try to compile git9 and adapt to my principia fork, use Claude!
no /sys/lib
Regexp lib without kencc-ext (so can be synced with the one in goken)
...??
...
**** fix pbs principia
- windows and rm segfault
- libap.a and NPROC
- fix fault when executing ~/longrun.c!

**** fix mk clean, segfault "corrupted size vs. prev_size
seems new

**** WEIRD: mk clean and mk nuke now has a segfault in rio/
omk does the same, so maybe pb in rc, or rm

pb seems to be 'rc' in goken, cos when MKSHELL=/usr/bin/rc works fine

**** WEIRD: mk fails in libap.a
unsetting NPROC helps
in case of pb mk nuke and retry helped too
...
*** take better screenshot with web browser (mothra?)
and editor (efuns?)
...
*** full pass on Make.nw and omk.nw
...
*** start from scratch from git9 code
...
**** add devtls.c to the kernel, and bind
and then make new screenshot of plan9 for the website
but running abaco on principia's website :) meta!

**** can https work? 9front is working with tls? need special config?
need MISC/BIG/kernel-port/devtls.c, ssh? need configure things?

would be nice to get it working because wikipedia is requiring https
for example

browser-engineering book assumes network library that can do TLS
(and js interpreter dukpy)
...
**** try to enable pics and fonts in mothra
DONE fonts by adding /lib/font/bit/dejavusans... from 9front
     pics ??
