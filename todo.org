# motivations: Knuth page! Pike 9fans! HN entries showing interest! GoT books! STEPS video?
...
- LPization hoc in Utilities.nw

- web testable! https://www.pcjs.org/ or vx
...
** Big missing pieces?
# database used to be one for example, but resolved now with chidb

The interesting remaining candidates are instead things where
Principia currently has the infrastructure/source, but hasn't yet made
the book:

- more Plan 9 network servers/protocol implementations
- more filesystem/storage programs
- perhaps document processing/typesetting
  but troff is ugly, maybe lout? or leave that to xix rewrite of lout?
- perhaps a particularly good crypto implementation
- perhaps a historically important Unix program that isn't represented by the Plan 9 ecosystem.

maybe upas?

or fossil/venti (userspace fs, snapshot, log, etc.)

or factotum (security as fs) and crypto stuff?

distributed system, RPC, etc. but 9p has it a bit.

** Later

*** LATER compile plan9 under plan9, 
...
*** LATER? move/merge with goken

*** LATER try get 9k kernel and more recent code from 9front

*** LATER incorporate richard miller pi2
# possibly easier now that have xv6 for it

and get it also working under QEMU and under my real pi2

make it work on raspberry pi 2, more interesting because SMP!
multiple CPUs so kernel book would be more interesting
...
*** LATER incorporate richard miller pi3, pi4, pi5 improvements!
# possibly easier now that have xv6 for it

and adapt to plan9 org and slightly modified kernel APIs

need also arm64 and bcm64 starting from pi3 so more complicated 
I also have a physical pi4!

*** LATER get it working on riscv orange pi!!
# possibly easier now if make xv6 first work for it

take inspiration from miller/geoff riscv kernel, or
more recently the 9front fork 

*** LATER get goken/principia working for my lattePC pi-like PC board
# possibly easier now if make xv6 first work for it
...
#update: use chatGPT/ClaudeCode to refine

finish 4 pairs? 5a/5l, lex/yacc, draw/rio, efuns/ocamlgit (ocaml books)

** still? priority 3: find publisher?
...
#alt: partially solved now with goken and synced code with principia
...
*** fix fault when executing ~/longrun.c!
...
**** try *test* the kernel, rio, etc. for pc under qemu in Dockerfile!
so really good regression test for goken
see test_386.rc exit code and output serial port trick?

**** test *test* the kernel for pi too
test_arm.rc!

need do 9pi2 instead of 9pi?
...
*** announce more widely 4 books
linked-in, Facebook, Twitter, Facebook Meta-eng
even maybe HN

no feedback on 9fans :(
maybe wait to have Make.nw and Shell.nw and Windows.nw then so at least 4 books


*** still? reduce number of books? so easier to sell to publishers? or make bundles?
...
**** add ref to classic research paper like for graphics with bresenham for all books
%classic-paper: ? 

and maybe add also
%classic-book:

**** make claude read the <prog>/docs/<prog>.pdf and possibly add explanations in corresponding books

**** RELAX more tags, tagging, and taggable sections

***** TODO-cs-history.txt

***** road-no-taken: list, TODO-road-not-taken.txt

***** why-win: list, TODO-why-win.txt

  - x86 over everything [Machine] — the wintel lock-in as network effect             
  - printf-debugging over formal debuggers [Debugger] — the dirty secret every survey
   confirms                                                                          
  - bash over other shells [Shell] — "GNU/Linux default. That's almost the entire  
  explanation."                                                                      
  - HTTP/REST over SOAP [Network] — testable with curl vs. requiring tooling         
  - C over Pascal [Compiler] — Unix shipped a C compiler; Pascal had no standard OS  
  interface                                                                          
  - IEEE 754 [Libcore] — ended floating-point chaos, won via Intel 8087 installed    
  base                                                                               
                                                                                   
***** comeback: list, TODO-comeback.txt

⏺ There it is. 25+ comeback arcs organized by book. Some highlights I'm most excited
  about:                                                                             
          
  - RISC vs CISC [Machine] — the Apple M1 vindication is dramatic                    
  - Modal editing [Editors] — Vim keybindings as the #1 VS Code extension is a great 
  data point                                                                         
  - Tiling window managers [Windows] — rio was ahead of its time                     
  - Green threads / M:N [Libcore] — Java Loom (2023) explicitly returning to what    
  libthread did in the 1990s                                                         
  - Record-and-replay debugging [Debugger] — rr bringing back a 1960s dream          
  - Immediate-mode GUI [Lib_gui] — Dear ImGui's success is a perfect comeback story  

**** reread and rerelease 9 green/yellow books!
after Profiler, ed, fully read Machine and release
do later same with the green one
=> will have 9 books in green/yellow zone, fully read, with index, released!


*** code indexing

**** make work mk index targets and process also includes like specified in the mkfile

**** still? syncweb: if GlobalDef and GlobalExtern, prefer GlobalDef 
ex: windows
but was fixed with renaming the chunk to windows (was window)

*** read other plan9 FAQ, and make claude read the FAQ
older one:
https://www.fywss.com/plan9/plan9v3faq.html
...
**** add reframe: tag to see linker and object files as first really
package/module system; 

- Symbol table as a contract: the .o file's symbol table is a
  promise ("I provide foo, I need bar"). The linker is a contract
  verifier that checks all promises are fulfilled.

**** add discussion in Linker/Assembler/Libcore? about frame pointer and ebp
and saving it and why kencc does not need? Advantage of frame pointer? 
For debugging? for stack trace?

see also PC-SP table!


should define what a "frame" is! in every book probably
...
**** Make.nw: mk index2 fail to find rcmp, Bufblock
weird cpp parsing hack error actually, about missing }
same pb with rc

**** test mk things with mk now synced in goken; go to principia/tests/mk/
and test hello.mk
easier to test under Linux and goken
...
*** still? more consistent type capitalized, and consistent use of typedefs
Code, Tree, etc

*** still? aspectize error management at more places

*** still? ctor, builtin, opcode
...
**** The original graphics papers — find them in Graphics.nw?
could add the ref in each relevant section with a special %classic-paper: tag?

%classic-paper:
Bresenham (1965) — line drawing algorithm, the most-implemented graphics algorithm ever
Pitteway & Watkinson (1980) — midpoint circle/ellipse algorithms
Cohen & Sutherland (1967) — the classic line clipping algorithm
Sutherland & Hodgman (1974) — polygon clipping
Xiaolin Wu (1991) — antialiased line drawing, still the standard

*** full pass Graphics book, good after Windows
rendu a Overview chapter
...
**** RELAX raw pass on Windows.nw

**** another full pass on Windows-1.0.pdf on remarkable? 
and then fix and produce Windows-1.1.pdf?

**** RELAX rio: replace 1, 2, 4 with LEFT MIDDLE RIGHT enums?

**** code: fix lens, statusbar, winwatch, find how to use them
lens generates error about screen

and try also window, wloc, label

**** try trigger autocomplete, Ctrl-F?? Insert triggers?

**** look at orio, need channel of channel, update orio.nw and resync and retest

****  reduce Windows_Extra.nw? remove some newlines? distribute more from Extra to Windows.nw?


*** try index rio with syncweb_indexer and get the index in Windows.pdf
...
**** improve Other kernels, especially LOC, compute them myself on software-src/
with codemap -loc

**** add a road-not-taken!
%road-not-taken:
- Multics rings: 8 protection rings with fine-grained access control.
  Unix collapsed them to 2 (kernel/user). The intermediate rings could
  have given us hardware-enforced privilege separation for libraries,
  drivers, and services — without the IPC cost of microkernels.
  (or even better use segmentation so each array/object different segment,
   no risk of overflow

**** read Kernel-12.pdf and annotate on remarkable, especially chapter-1-2

**** => release Kernel-13.pdf on website!

**** Kernel_extra.nw cleaning and distribute in Kernel.nw


*** RELAX go through Appendix E, Kernel.nw, Extra code, and Kernel_extra.nw

*** Debugging appendix Kernel, use special keys so can see size of Proc for instance
and maybe reference it from previous chapter
...
reframe:
- malloc as an OS within the process: malloc manages a heap the
  way the kernel manages physical memory — free lists, splitting,
  coalescing, fragmentation. It's a memory manager inside a
  memory manager.

**** Refine _main, explain better, aspectize and move maybe to further sections

**** Libcore.nw: malloc chapter and aspectize debugging, checks, etc

**** Libcore.nw: pass on part2, data processing

**** Libcore.nw: pass on part3, System services

**** Libcore: could maybe add zlib to lineage? and also regexps libs? google-re?
gmalloc too, jemalloc, etc?
...
*** learn to use snoopy
and cleanup MISC/BIG/snoopy which seems to have file redundant
with networking/snoopy/

*** less: helloworlds.c using dial()
and then look at code of dial
...
even more relevant in AI time! claude code is using find,
sed, tr, awk, grep, ...
in very nice ways too

*** continue hoc

*** start lp split awk!

*** add %claude explanations to Utilities.nw! start with %yoann first?
augment ~/.claude/CLAUDE.md then

*** lpize awk, resume Utilities.nw, maybe start %claude annots there
...
** VCS.nw
# no AI for this one, for respect for Ori (even though he does not deserve it)

*** Last

**** finish LP split branch.rc

**** diff, patch, merge3, distribute from VCS_extra.nw
so have smaller Extra code chapter
right now still at 

*** VCS overview/principles? 
...
https://www.9lab.org/plan9/debugging-with-acid/

**** ----------------------------------------------

**** add a Debugger.nw why-win:
- printf-debugging over formal debuggers: zero setup, works in any language,
  works across network boundaries, works in production, doesn't change timing
  much (vs breakpoints that stop the world). Every survey shows most
  developers use printf most of the time despite having GDB/LLDB available.

**** add de Debugger.nw reframe: for program-reading
- Debugger as program-reading tool, not bug-finding tool: the
  debugger's primary value is making execution visible. It's a
  microscope for programs — you learn more by watching a program
  run than by reading its source.

**** => release new Debugger version, should pass 25% limit now

**** RELAX LP distribute more functions from acid, or move in proper
appendix about additional acid builtins?

go again through Debugger_extra.nw and reduce by distributing

**** fix Debugger-3.pdf remarkable annotations part2 (long)

**** RELAX LP split whatis() and distribute

**** try db on hello_bug.c, check section in appendix

**** try lstk() in acid on hello_bug
and add in relevant section

**** --------------------------------------------------------

**** try acid on plan9port or goken and check DWARF and ELF support
...
*** reread fully and release Profiler, pass now 50% limit

*** profiler: explain how to get time of child and all its children
complicated on Linux actually, cf semgrep, but in plan9
children write in Waitmsg parent the CPU time used

say ticks in Kernel support for CPU time, and wall time = system time
...
*** maybe relate opcode interpreter with ARM dissassembler in Debugger?
can cross-help each other? explanations to add?
...
**** Browser: finish chapter 4 split

**** Browser: LP split chapter 5 (url parsing)

**** Browser: LP split chapter 6 (hget)

**** Browser.nw: start aspectize advanced features/topics
- ftp
- TODO opti?

**** redo explanation pass, the "proactive" one with ASCII diagram
read CLAUDE.md and MEMORY.md

**** add htmlfmt/ to browsers/

*** start lpize abaco? webfs? libwww? html.h? url.h? or mothra?
and add on website! can start with webfs
...
take its miniyacc.c? instead of unix yacc.c ? (1000LOC vs 3000LOC)

**** skeleton at least of Generator.nw and generate v0 pdf
to put on website
and mention in Principia.nw and LOC code orga tabular


** Compiler HLL (now in OCaml.nw)

** Typesetting.nw

*** try to compile 'page'? but need ghostscript too?
...
*** less: try to improve 'stats' program
better memory baseline, more useful lines
...
*** BUG mothra (and abaco too) freezes on https://wikipedia.org pretty quickly
claude --resume 88f56f71-b246-4438-b4ca-663c21ce0d27


*** Browser

**** TEDIOUS test_resolve_url.c test example that call webclone, urlresolved, etc.
independently of mothra so easier to test quickly

or just modify webfsget.c example

/mnt/web
also look at code of abaco doing the same?

**** LPize webfs, webcookies, hget.c, webfsget.c, and abaco/mothra in Browser.nw
libhtml? not used by mothra so maybe less important

** plan9

*** add sdb in principia? under database/
why not


*** less: add tree to plan9? I use it in VCS.nw Repository format section


*** now that have troff, can make 'man' to work?
need more fonts?

need /sys/man so need move docs/man/ to ROOT/
...
*** BUG? running test_bind_c replace / also for parent process!
wtf!
seems to be ok when run inside rio, at least separate windows have
different namespace, but in the same window executing
test_bind_c will also modify the namespace of the parent.

or maybe intended? not a bug? rfork default behavior?


** 5l

*** detect when link and multiple functions are matching, like
bind in libbsd.a and lib9.a
...
SEMI pichardie, ridoux
fradet, banatre
SEMI jarod, alok, julien
SEMI pike, knuth
russ cox
csapp authors
...
** Compiler frontend (tiger), see fork-tiger/todo.org

** Compiler backend (c--), see fork-c--/todo.org

** xv6, see fork-xv6/todo.org
